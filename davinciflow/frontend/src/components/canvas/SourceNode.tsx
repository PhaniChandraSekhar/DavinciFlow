...
type SourceNodeType = Node<PipelineNodeData, 'sourceNode'>;
...
export default function SourceNode({ data, selected }: NodeProps<SourceNodeType>) {
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="flex items-start gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-blue-500/30 bg-blue-500/15 text-blue-400">
            <StepGlyph icon={nodeData.definition?.icon ?? 'file'} className="h-4 w-4" />
          </div>
      ...
      <Handle type="source" position={Position.Bottom} className="!border-blue-500/70 !bg-slate-950" />
    </div>
  );
}
