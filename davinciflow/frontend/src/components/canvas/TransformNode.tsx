...
type TransformNodeType = Node<PipelineNodeData, 'transformNode'>;
...
export default function TransformNode({ data, selected }: NodeProps<TransformNodeType>) {
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Top} className="!border-purple-500/70 !bg-slate-950" />
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="flex items-start gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-purple-500/30 bg-purple-500/15 text-purple-400">
            <StepGlyph icon={nodeData.definition?.icon ?? 'settings'} className="h-4 w-4" />
          </div>
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold text-slate-100">{nodeData.label}</div>
            <div className="text-[10px] uppercase tracking-widest text-slate-500">Transform</div>
          </div>
        </div>
      ...
      <Handle type="source" position={Position.Bottom} className="!border-purple-500/70 !bg-slate-950" />
    </div>
  );
}
